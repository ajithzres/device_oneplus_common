...
public class KeyHandler implements DeviceKeyHandler {
    private String TAG = "OPKeyHandler-KeyHandler";
    ...
    public KeyHandler(Context context) {
        ...
        mProximityTimeOut = resources.getInteger(com.android.internal.R.integer.config_proximityCheckTimeout);
        mProximityWakeSupported = resources.getBoolean(com.android.internal.R.bool.config_proximityCheckOnWake);
    ...
    private class EventHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            switch (message.arg1) {
                case Constants.KEY_DOUBLE_TAP:
                case Constants.KEY_GESTURE_CIRCLE:
                    ...
                    doHapticFeedback();
                    break;
                case Constants.KEY_GESTURE_TWO_SWIPE:
                    dispatchMediaKeyWithWakeLockToMediaSession(KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE);

                    doHapticFeedback();
                    break;
                case Constants.KEY_GESTURE_DOWN_ARROW:
                    ...
                    break;
                case Constants.KEY_GESTURE_LEFT_V:
                    dispatchMediaKeyWithWakeLockToMediaSession(KeyEvent.KEYCODE_MEDIA_PREVIOUS);

                    doHapticFeedback();
                    break;
                case Constants.KEY_GESTURE_RIGHT_V:
    ...
    public KeyEvent handleKeyEvent(KeyEvent event) {
        ...
        boolean isKeySupported = ArrayUtils.contains(Constants.sSupportedGestures, scanCode);
        boolean isSliderModeSupported = Constants.sSupportedSliderModes.indexOfKey(scanCode) >= 0;
        if (!isKeySupported && !isSliderModeSupported) {
            return event;
        }

        // We only want ACTION_UP event, except KEY_DOUBLE_TAP
        if (scanCode == Constants.KEY_DOUBLE_TAP) {
        ...
        if (isSliderModeSupported) {
            if (scanCode == Constants.KEY_SLIDER_MODE_VIBRATION) {
                mNotificationManager.setZenMode(Settings.Global.ZEN_MODE_OFF, null, TAG);
                mAudioManager.setRingerModeInternal(AudioManager.RINGER_MODE_VIBRATE);
            } else {
                mAudioManager.setRingerModeInternal(AudioManager.RINGER_MODE_NORMAL);
                mNotificationManager.setZenMode(Constants.sSupportedSliderModes.get(scanCode), null, TAG);
            }
			...
			if (mProximitySensor != null && proximityWakeCheckEnabled && mProximityWakeSupported) {
                mEventHandler.sendMessageDelayed(message, mProximityTimeOut);
    ...
    private void doHapticFeedback() {
        ...
        boolean enabled = (boolean) Constants.getPreferenceValue(mContext.getApplicationContext(), Constants.TOUCHSCREEN_GESTURE_HAPTIC_FEEDBACK_KEY);
        if (enabled) {
